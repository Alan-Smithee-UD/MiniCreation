...
function sendInput() {
    ...
    if (previous) {
        ...
        // 直前のものの最後の文字を取得
        const preLastChar = getLastChar(previous);
        if (inputFirstChar != preLastChar) {
            alert('文字が繋がっていません');
            return;
        }
    }
    if (previous) {
        history.innerText = `あなた：${input}\n↑\n${history.innerText}`;
    } else {
        history.innerText = `あなた：${input}`;
    }
    deleteElementFromList(input);
    // 次のポケモンを取得
    ...
    previous = next;
    inputBox.value = ''
    deleteElementFromList(next);
}

function getNextPoke(startChar) {
    const availablePokemons = list.filter((pokemon) => {
        return pokemon.charAt(0) === startChar && pokemon.charAt(pokemon.length - 1) !== 'ン';
    });
...
function getLastChar(str) {
    ...
    if (smallToLargeMap[lastChar]) {
        return smallToLargeMap[lastChar]; // 小文字カタカナを大文字に変換
    }

    // それ以外の場合はそのまま最後の文字を返す
    return lastChar;
}

function deleteElementFromList(target) {
